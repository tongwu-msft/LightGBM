...
namespace LightGBM {
  ...
  void TargetEncoder::SetPrior(const double /*prior*/, const double prior_weight) {
    prior_weight_ = prior_weight;
  }

  std::string TargetEncoder::FeatureName() const {
  ...
  json11::Json TargetEncoder::DumpToJSONObject() const {
  ...
  std::string TargetEncoder::DumpToString() const {
    std::stringstream str_stream;
    Common::C_stringstream(str_stream);
